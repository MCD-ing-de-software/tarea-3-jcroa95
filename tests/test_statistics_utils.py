...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        utils= StatisticsUtils()
        arra=[1.0, 2.0, 3.0, 4.0]
        ventana=2
        result=utils.moving_average(arra, window=2)

        expected = np.array([1.5, 2.5, 3.5])

        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)

        #El largo del array - su ventana +1 sería generalizar el numero obtenido para shape que debiese tener el resultado
        self.assertEqual(result.shape, (len(arra)-ventana+1,),"El resultado debería tener el shape esperado")
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        with self.assertRaises(ValueError):
            utils.moving_average(arra, window=0)

        with self.assertRaises(ValueError):
            utils.moving_average(arra, window=len(arra)+1)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        result=utils.zscore(arr=arra)

        self.assertAlmostEqual(result.mean(),0.0, places=10,
                               msg="La media de los z-scores debería ser 0")
        self.assertAlmostEqual(result.std(),1.0, places=10,
                               msg="La desviación estándar de los z-scores debería ser 1")
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        self.assertAlmostEqual(result.min(),0.0, places=10,
                               msg="El minimo del array resultante debe ser 0")

        self.assertAlmostEqual(result.max(),1.0, places=10,
                               msg="El máximo del array resultante debe ser 0")
